...
class Group(models.Model):
    ...
    owner_id = models.IntegerField(unique=True, default=False)
    group_id = models.IntegerField(default=False, unique=True)
    ...
    def get_absolute_url(self):
        return reverse("group", args=[str(self.group_id)])
...
class Message(models.Model):
    ...
    def __str__(self) -> str:
        ...
        return f"{self.author}:- {self.content} @{date} {time.hour}:{time.minute}:{time.second}"
